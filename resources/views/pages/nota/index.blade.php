        ...
        <div class="container">
            ...
            <div class="page-header">
                <div class="page-leftheader">
                    <h4 class="page-title">List Nota Kesepahaman</h4>
                </div>
                <div class="page-rightheader ml-auto d-lg-flex d-none">
                    <ol class="breadcrumb">
                        <li class="breadcrumb-item"><a href="#" class="d-flex"><svg class="svg-icon" xmlns="http://www.w3.org/2000/svg" height="24" viewBox="0 0 24 24" width="24"><path d="M0 0h24v24H0V0z" fill="none"/><path d="M12 3L2 12h3v8h6v-6h2v6h6v-8h3L12 3zm5 15h-2v-6H9v6H7v-7.81l5-4.5 5 4.5V18z"/><path d="M7 10.19V18h2v-6h6v6h2v-7.81l-5-4.5z" opacity=".3"/></svg><span class="breadcrumb-icon"> Home</span></a></li>
                        <li class="breadcrumb-item"><a href="{{ route('admin.nota.index') }}">List Nota Kesepahaman</a></li>
                    </ol>
                </div>
            </div>
            <!--End Page header-->
            ...
            <div class="row">
                                ...
                                <table class="table table-bordered table-striped text-nowrap mb-0" id="example1">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td>{{ $nota->rs_name }}</td>
                                            <td>
                                                <img src="{{ asset('uploads/'.$nota->rs_logo) }}" alt="" style="width:20px">
                                                
                                            </td>
                                            
                                            <td class="text-right">
                                                <a href="{{ route('admin.nota.cetak', ['id' => $nota->id]) }}">
                                                    <button class="btn btn-sm btn-primary">Cetak</button>
                                                </a>
                                                <a href="{{ route('admin.nota.edit', ['id' => $nota->id]) }}">
                                                    <button class="btn btn-sm btn-success">Edit</button>
                                                </a>
    
                                                <button type="button" class="btn btn-sm btn-danger" data-toggle="modal"
                                                    data-target="#delete{{ $nota->id }}">
                                                    Delete
                                                </button>
    
                                                <!-- Modal -->
                                                <form action="{{ route('admin.nota.destroy', ['id' => $nota->id]) }}"
                                                    method="POST"class="d-inline">
                                                    @csrf
                                                    @method('DELETE')
                                                    <div class="modal fade" id="delete{{ $nota->id }}" tabindex="-1"
                                                        ...
                                                        <div class="modal-dialog" role="document">
                                                            <div class="modal-content">
                                                                ...
                                                                <div class="modal-body text-left">
                                                                    Apakah Anda ingin menghapus data <b>{{ $nota->rs_name }}</b>
                                                                    ?
                                                                </div>
                ...
                <div class="col-lg-4">
                    <div class="card">
                        <div class="card-header">
                            <h4 class="card-title">Tambah Nota Kesepahaman</h4>
                        </div>
                        <div class="card-body">
                            <form action="{{ @$edit_mode ? route('admin.nota.update', ['id' => $detail->id]) : route('admin.nota.store') }}" method="POST" enctype="multipart/form-data">
                                @csrf
                                @if (@$edit_mode)
                                @method('PUT')
                                @endif
                                <div class="">
                                    <div class="form-group">
                                        <label for="rs_logo" class="form-label">Logo :</label>
                                        <input type="file" name="rs_logo" class="dropify" id="rs_logo" placeholder="rs_Logo"  data-default-file="{{ @$edit_mode ? asset('uploads/' . $detail->rs_logo) : '' }}">
                                        @error('rs_logo')
                                            <label id="logo-error" class="error mt-2 text-danger" for="rs_logo">{{$message}}</label>
                                        @enderror
                                    </div>
                                    <div class="form-group">
